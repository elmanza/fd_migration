...
class RwFdSynchronize {
    ...
    async refreshRWQuoteSyncTask() {
        if (!this.finishedProcess.refreshRWQuoteSyncTask) {
            return null;
        }
        Logger.info((new Date()).toString() + "refreshRWQuoteSyncTask task is called.");
        this.finishedProcess.refreshRWQuoteSyncTask = false;
        ...
        let processRecords = async (page) => {
            Logger.info(`refreshRWQuoteSyncTask processRecords Batch: ${page}`);
            ...
            for (let index in records) {
                ...
                refreshProcess.push(new Promise((resolve, reject) => {
                    this.refreshRWEntity(stageQuote)
                        .then(result => {
                            Logger.info({
                                message: `refreshRWQuoteSyncTask ${stageQuote.fdOrderId} (Batch: ${page} index: ${index})`,
                                result
                            });
                            resolve(true);
                        })
                        .catch(error => {
                            Logger.error(`refreshRWQuoteSyncTask ${stageQuote.fdOrderId} Error ${stageQuote.fdOrderId} :  ${error.message} (Batch: ${page} index: ${index})`);
                            Logger.error(error);
                            resolve(error);
                        });
                }));
            ...
            Logger.info(`refreshRWQuoteSyncTask processRecords Batch: ${page} FINISHED`);
        };
        ...
        this.finishedProcess.refreshRWQuoteSyncTask = true;
        Logger.info((new Date()).toString() + "refreshRWQuoteSyncTask task is FINISHED.");
    }
